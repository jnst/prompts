 ...
 */
export async function detectAllOutputFiles(promptPath) {
    // Validate inputs
    if (!promptPath || typeof promptPath !== 'string') {
        return err(new ValidationError('Prompt path', 'a string'));
    }
    ...
    try {
        ...
        return err(new OutputWriteError(outputsDir, `Failed to scan output files: ${error}`));
    }
}
/**
 ...
 */
export async function detectFilledFiles(promptPath) {
    // Validate inputs
    if (!promptPath || typeof promptPath !== 'string') {
        return err(new ValidationError('Prompt path', 'a string'));
    }
    ...
    try {
        ...
        return err(new OutputWriteError(outputsDir, `Failed to scan filled files: ${error}`));
    }
}
/**
 * Clear file content while preserving frontmatter (if exists)
 */
export async function clearFileContent(filePath) {
    // Validate inputs
    if (!filePath || typeof filePath !== 'string') {
        return err(new ValidationError('File path', 'a string'));
    }
    try {
        ...
        return err(new OutputWriteError(filePath, `Failed to clear file: ${error}`));
    }
}
/**
...
export async function removeFile(filePath) {
    ...
    try {
        ...
        return err(new OutputWriteError(filePath, `Failed to remove file: ${error}`));
    }
}
